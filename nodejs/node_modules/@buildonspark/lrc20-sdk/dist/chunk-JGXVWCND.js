...
function publicKeyToPayment(publicKey, type, networkType) {
  ...
  if (type === 0 /* P2PKH */) {
    return bitcoin2.payments.p2pkh({
      pubkey,
      network
    });
  } else if (type === 1 /* P2WPKH */ || type === 4 /* M44_P2WPKH */) {
    return bitcoin2.payments.p2wpkh({
      pubkey,
      network
    });
  } else if (type === 2 /* P2TR */ || type === 5 /* M44_P2TR */) {
    return bitcoin2.payments.p2tr({
      pubkey: toXOnly(pubkey),
      network
    });
  } else if (type === 3 /* P2SH_P2WPKH */) {
    const data = bitcoin2.payments.p2wpkh({
      pubkey,
      network
    });
    return bitcoin2.payments.p2sh({
      pubkey,
      network,
      redeem: data
    });
...
function addressToScriptPk(address3, networkType) {
  const network = toPsbtNetwork(networkType);
  return bitcoin2.address.toOutputScript(address3, network);
}
function isValidAddress(address3, networkType = 0 /* MAINNET */) {
  let error;
  try {
    bitcoin2.address.toOutputScript(address3, toPsbtNetwork(networkType));
  } catch (e) {
    error = e;
  }
  if (error) {
...
function decodeAddress(address3) {
  const mainnet = bitcoin2.networks.bitcoin;
  const testnet = bitcoin2.networks.testnet;
  const regtest = bitcoin2.networks.regtest;
  let decodeBase58;
  let decodeBech32;
  let networkType = 0 /* MAINNET */;
  let addressType = 8 /* UNKNOWN */;
  if (address3.startsWith("bc1") || address3.startsWith("tb1") || address3.startsWith("bcrt1")) {
    try {
      decodeBech32 = bitcoin2.address.fromBech32(address3);
    ...
    try {
      decodeBase58 = bitcoin2.address.fromBase58Check(address3);
...
function getAddressType(address3, networkType = 0 /* MAINNET */) {
  return decodeAddress(address3).addressType;
}
function scriptPkToAddress(scriptPk, networkType = 0 /* MAINNET */) {
  const network = toPsbtNetwork(networkType);
  try {
    const address3 = bitcoin2.address.fromOutputScript(
...
var ElectrsApi = class {
  ...
  async listBitcoinUtxo(address3) {
    const url = `${this.electrsUrl}/address/${address3}/utxo`;
    try {
      const response = await fetch(url, {
        method: "GET",
        headers: {
  ...
  async listTransactions(address3) {
    const url = `${this.electrsUrl}/address/${address3}/txs`;
    const response = await fetch(url, {
      method: "GET",
      headers: {
        ...this.auth ? { Authorization: `Basic ${basicAuth(this.auth)}` } : {}
...
import { Psbt, payments as payments2, Transaction, script, opcodes } from "bitcoinjs-lib";
  ...
  outputToPayment(output) {
    ...
    switch (output.type) {
      ...
      case "MultisigReceiptOutput":
        ...
        break;
      case "OPReturnOutput":
        payment = payments2.embed({ data: output.data });
        break;
...
var DefaultTokenSigner = class {
  ...
  getReceiptPrivateKey(receipt) {
    ...
    const pxhPubkey = bitcoin3.crypto.sha256(Buffer.concat([pxh, innerKey]));
...
// src/lrc/wallet/wallet.ts
var { address: address2, crypto: bitcoinJsCrypto, payments: payments3, Psbt: Psbt2, script: script2, Transaction: Transaction2 } = bitcoin;
var LRCWallet = class _LRCWallet {
  ...
  async syncWallet() {
    const bech32Address = payments3.p2wpkh({
      pubkey: this.pubkey,
      network: this.network
    }).address;
  ...
  async fetchUtxos(address3) {
    let txs = await this.electrsApi.listTransactions(address3);
    let inputs = [];
    ...
    txs.forEach((tx) => {
      ...
      for (let i = 0; i < tx.vout.length; i++) {
        if (tx.vout[i].scriptpubkey_address == address3) {
  ...
  async prepareTakerPsbt(giveData, receiveData, allowSendMore = false, feeRateVb, total_tx_fee) {
    ...
    const p2trAddress = payments3.p2tr({
      network: this.network,
  ...
  async signPsbt(takerTxs, makerTx, feeRateVb) {
    ...
    for (const transaction of takerTxs) {
      const txHash2 = Buffer.from(transaction.bitcoin_tx.ins[0].hash);
      ...
      psbt.addInput({
        hash: txHash2,
        index: transaction.bitcoin_tx.ins[0].index,
        nonWitnessUtxo: Buffer.from(txHex2, "hex")
      });
      if (transaction.tx_type.type === "Transfer" /* Transfer */) {
        const input_proof = transaction.tx_type.data.input_proofs.get(0);
        if (input_proof.type === "Sig" /* Sig */) {
    ...
    for (let i = 0; i < transactions.length; i++) {
      if (transactions[i].tx_type.type == "Transfer" /* Transfer */) {
        ...
        if (output_proof.type = "Sig" /* Sig */) {
          const tweakedPubKey = Receipt.receiptPublicKey(
            Buffer.from(output_proof.data.innerKey, "hex"),
  ...
  createOutputs(amounts) {
    ...
    return amounts.map((token) => {
      ...
      if (token.metadata) {
        ...
        const addressParsed = address2.fromBech32(token.recipient);
  ...
  pubkeyPositionInScript(pubkey, outScript) {
    ...
    const decompiled = script2.decompile(outScript);
  ...
  async signRawTransaction(hex) {
    let unsignedTx = Transaction2.fromHex(hex);
