...
__export(index_exports, {
  ...
  AnnouncementDataType: () => AnnouncementDataType,
  BLINDING_FACTOR_SIZE: () => BLINDING_FACTOR_SIZE,
  BitcoinTransactionDto: () => BitcoinTransactionDto,
  BitcoinUtxo: () => BitcoinUtxo,
  BitcoinUtxoSpentStatus: () => BitcoinUtxoSpentStatus,
  BitcoinUtxoStatus: () => BitcoinUtxoStatus,
  BtcUtxosCoinSelection: () => BtcUtxosCoinSelection,
  DUST_AMOUNT: () => DUST_AMOUNT,
  ELECTRS_URL: () => ELECTRS_URL,
  EMPTY_TOKEN_PUBKEY: () => EMPTY_TOKEN_PUBKEY,
  ElectrsApi: () => ElectrsApi,
  EmptyReceiptProofDataDto: () => EmptyReceiptProofDataDto,
  ...
  JSONStringifyBodyDown: () => JSONStringifyBodyDown,
  LRCWallet: () => LRCWallet,
  LRC_NODE_URL: () => LRC_NODE_URL,
  LightningCommitmentProofDataDto: () => LightningCommitmentProofDataDto,
  LightningHtlcProofDataDto: () => LightningHtlcProofDataDto,
  Lrc20JsonRPC: () => Lrc20JsonRPC,
  Lrc20Transaction: () => Lrc20Transaction,
  Lrc20TransactionDto: () => Lrc20TransactionDto,
  Lrc20TransactionParser: () => Lrc20TransactionParser,
  Lrc20TransactionStatus: () => Lrc20TransactionStatus,
  Lrc20TransactionTypeDto: () => Lrc20TransactionTypeDto,
  Lrc20TransactionTypeEnum: () => Lrc20TransactionTypeEnum,
  Lrc20Utxo: () => Lrc20Utxo,
  MIN_DUST_AMOUNT: () => MIN_DUST_AMOUNT,
  MultisigReceiptProofDataDto: () => MultisigReceiptProofDataDto,
  NetworkType: () => NetworkType,
  P2WSHProofDataDto: () => P2WSHProofDataDto,
  PARITY: () => PARITY,
  PubkeyFreezeAnnouncement: () => PubkeyFreezeAnnouncement,
  Receipt: () => Receipt,
  ReceiptProofDto: () => ReceiptProofDto,
  ReceiptProofType: () => ReceiptProofType,
  SigReceiptProofDataDto: () => SigReceiptProofDataDto,
  SparkExitProofDataDto: () => SparkExitProofDataDto,
  TOKEN_AMOUNT_SIZE: () => TOKEN_AMOUNT_SIZE,
  TokenPubkey: () => TokenPubkey,
  TokenPubkeyAnnouncement: () => TokenPubkeyAnnouncement,
  TokenPubkeyInfo: () => TokenPubkeyInfo,
  TransactionBuilder: () => TransactionBuilder,
  TransferOwnershipAnnouncement: () => TransferOwnershipAnnouncement,
  TxFreezeAnnouncement: () => TxFreezeAnnouncement,
  addressToScriptPk: () => addressToScriptPk,
  basicAuth: () => basicAuth,
  createMethodDecorator: () => createMethodDecorator,
  decodeAddress: () => decodeAddress,
  filterUniqueUtxo: () => filterUniqueUtxo,
  findNotFirstUsingFind: () => findNotFirstUsingFind,
  fromPrivateKey: () => fromPrivateKey,
  getAddressType: () => getAddressType,
  getNetwork: () => getNetwork,
  getReceiptDataFromProof: () => getReceiptDataFromProof,
  isValidAddress: () => isValidAddress,
  network: () => network,
  networks: () => networks2,
  parseAnnouncementData: () => parseAnnouncementData,
  publicKeyToAddress: () => publicKeyToAddress,
  publicKeyToPayment: () => publicKeyToPayment,
  publicKeyToScriptPk: () => publicKeyToScriptPk,
  reverseBuffer: () => reverseBuffer,
  scriptPkToAddress: () => scriptPkToAddress,
  toEvenParity: () => toEvenParity,
  toNetworkType: () => toNetworkType,
  toPsbtNetwork: () => toPsbtNetwork,
  toXOnly: () => toXOnly
});
...
// src/lrc/utils/constants.ts
var G = import_buffer6.Buffer.from("0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798", "hex");
...
var DUST_AMOUNT = 354;
var PARITY = import_buffer6.Buffer.from([2]);
var EMPTY_TOKEN_PUBKEY = import_buffer6.Buffer.from(Array(33).fill(2));
...
var AddressType = /* @__PURE__ */ ((AddressType2) => {
  ...
  AddressType2[AddressType2["M44_P2WPKH"] = 4] = "M44_P2WPKH";
  AddressType2[AddressType2["M44_P2TR"] = 5] = "M44_P2TR";
  AddressType2[AddressType2["P2WSH"] = 6] = "P2WSH";
  AddressType2[AddressType2["P2SH"] = 7] = "P2SH";
  AddressType2[AddressType2["UNKNOWN"] = 8] = "UNKNOWN";
  return AddressType2;
})(AddressType || {});
...
function publicKeyToPayment(publicKey, type, networkType) {
  ...
  if (type === 0 /* P2PKH */) {
    return bitcoin3.payments.p2pkh({
      pubkey,
      network: network2
    });
  } else if (type === 1 /* P2WPKH */ || type === 4 /* M44_P2WPKH */) {
    return bitcoin3.payments.p2wpkh({
      pubkey,
      network: network2
    });
  } else if (type === 2 /* P2TR */ || type === 5 /* M44_P2TR */) {
    return bitcoin3.payments.p2tr({
      pubkey: toXOnly(pubkey),
      network: network2
    });
  } else if (type === 3 /* P2SH_P2WPKH */) {
    const data = bitcoin3.payments.p2wpkh({
      pubkey,
      network: network2
    });
    return bitcoin3.payments.p2sh({
      pubkey,
      network: network2,
      redeem: data
    });
...
function addressToScriptPk(address5, networkType) {
  const network2 = toPsbtNetwork(networkType);
  return bitcoin3.address.toOutputScript(address5, network2);
}
function isValidAddress(address5, networkType = 0 /* MAINNET */) {
  let error;
  try {
    bitcoin3.address.toOutputScript(address5, toPsbtNetwork(networkType));
  } catch (e) {
    error = e;
  }
  if (error) {
...
function decodeAddress(address5) {
  const mainnet = bitcoin3.networks.bitcoin;
  const testnet = bitcoin3.networks.testnet;
  const regtest = bitcoin3.networks.regtest;
  let decodeBase58;
  let decodeBech32;
  let networkType = 0 /* MAINNET */;
  let addressType = 8 /* UNKNOWN */;
  if (address5.startsWith("bc1") || address5.startsWith("tb1") || address5.startsWith("bcrt1")) {
    try {
      decodeBech32 = bitcoin3.address.fromBech32(address5);
    ...
    try {
      decodeBase58 = bitcoin3.address.fromBase58Check(address5);
  ...
  return {
    networkType: 0 /* MAINNET */,
    addressType: 8 /* UNKNOWN */,
    dust: 546
  };
}
function getAddressTypeDust(addressType) {
  if (addressType === 1 /* P2WPKH */ || addressType === 4 /* M44_P2WPKH */) {
    return 294;
  } else if (addressType === 2 /* P2TR */ || addressType === 5 /* M44_P2TR */) {
    return 330;
  } else {
    return 546;
  }
}
function getAddressType(address5, networkType = 0 /* MAINNET */) {
  return decodeAddress(address5).addressType;
}
function scriptPkToAddress(scriptPk, networkType = 0 /* MAINNET */) {
  const network2 = toPsbtNetwork(networkType);
  try {
    const address5 = bitcoin3.address.fromOutputScript(
      typeof scriptPk === "string" ? import_buffer.Buffer.from(scriptPk, "hex") : scriptPk,
      network2
    );
    return address5;
  } catch (e) {
...
var import_secp256k1 = require("@bitcoinerlab/secp256k1");
...
var Receipt = class _Receipt {
  ...
  static receiptHash(receipt) {
    ...
    const yHash = bitcoin4.crypto.sha256(import_buffer.Buffer.from(y.toBytes()));
    const uvInner = uv.pubkey || EMPTY_TOKEN_PUBKEY;
    const pxh = bitcoin4.crypto.sha256(import_buffer.Buffer.concat([yHash, import_buffer.Buffer.from(uvInner)]));
    return pxh;
  }
  static receiptPublicKey(innerKey, receipt) {
    const pxh = _Receipt.receiptHash(receipt);
    const pxhPubkey = bitcoin4.crypto.sha256(import_buffer.Buffer.concat([pxh, innerKey]));
    const pxhPubkeyPoint = (0, import_secp256k1.pointMultiply)(G, pxhPubkey);
    const receiptKey = (0, import_secp256k1.pointAdd)(pxhPubkeyPoint, innerKey);
...
var Lrc20TransactionParser = class {
  static stringify(tx) {
    return JSON.stringify((0, import_class_transformer3.instanceToPlain)(tx), (key, value) => typeof value === "bigint" ? Number(value) : value);
  }
  static fromString(stringTx) {
    let lrc20Tx = JSON.parse(stringTx);
    lrc20Tx.bitcoin_tx.ins.forEach((input) => {
      input.hash = import_buffer.Buffer.from(input.hash);
      input.script = import_buffer.Buffer.from(input.script);
      const witnessCasted = [];
      input.witness.map((witness) => {
        witnessCasted.push(import_buffer.Buffer.from(witness));
      });
      input.witness = witnessCasted;
    });
    lrc20Tx.bitcoin_tx.outs.forEach((output) => {
      output.script = import_buffer.Buffer.from(output.script);
    });
  ...
  static toHex(tx) {
    return import_buffer.Buffer.from(this.stringify(tx)).toString("hex");
  }
  static fromHex(hex) {
    return this.fromString(import_buffer.Buffer.from(hex, "hex").toString());
  }
  static castReceiptProof(receiptProof) {
    switch (receiptProof.type) {
      case "Sig" /* Sig */:
        receiptProof.data.receipt.tokenPubkey = new TokenPubkey(
          import_buffer.Buffer.from(receiptProof.data.receipt.tokenPubkey.pubkey)
        );
...
function makeKeyPair(priv, pub, network2) {
  ...
  const publicKey = pub ?? import_buffer.Buffer.from(import_secp256k12.secp256k1.getPublicKey(toU8(priv), true));
  const sign = (hash) => {
    if (!priv) throw new Error("Private key required for signing");
    const sigObj = import_secp256k12.secp256k1.sign(toU8(hash), toU8(priv));
  ...
  const signSchnorr = (hash) => {
    ...
    return import_buffer.Buffer.from(import_secp256k12.schnorr.sign(toU8(hash), toU8(priv)));
  };
  const verify = (hash, signature) => {
    try {
      const sig = import_secp256k12.secp256k1.Signature.fromDER(toU8(signature));
      return import_secp256k12.secp256k1.verify(sig, toU8(hash), toU8(publicKey));
    } catch {
      return false;
    }
  };
  const verifySchnorr = (hash, signature) => import_secp256k12.schnorr.verify(toU8(signature), toU8(hash), toU8(publicKey));
  return {
    publicKey,
    privateKey: priv,
    network: network2,
...
// src/lrc/wallet/wallet.ts
var { address: address4, crypto: bitcoinJsCrypto, payments: payments4, Psbt: Psbt2, script: script3, Transaction: Transaction3 } = bitcoin5;
