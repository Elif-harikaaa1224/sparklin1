...
if not TELEGRAM_TOKEN:
    ...
    exit(1)
wallet_manager = SparkWalletManager("./spark_wallets")
user_passwords: dict = {}  # Временное хранилище паролей в памяти

# Инициализируем wallet_manager в withdrawal handlers
set_wallet_manager(wallet_manager)

# Хранение позиций по мемам
POSITIONS_FILE = "./spark_wallets/positions.json"
...
def add_position(wallet_name: str, contract_address: str, amount_sats: int, action: str):
...
def get_positions(wallet_name: Optional[str] = None) -> dict:
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if not wallets:
        try:
            ...
            wallet_manager.set_master_password(default_password)
            result = wallet_manager.create_new_wallet(wallet_name)
            
            user_passwords[user_id] = default_password
            
            # Устанавливаем как активный
            wallet_manager.set_active_wallet(wallet_name)
...
async def cmd_my_wallets(message: types.Message):
    ...
    if not wallets:
        ...
        active_wallet_name = wallet_manager.active_wallet
        ...
        for idx, (name, info) in enumerate(wallet_list, 1):
            ...
            try:
                from btc_price import btc_price_service
                balance_info = await wallet_manager.get_wallet_balance(address)
                balance_sats = balance_info.get("balance_sats", 0)
...
async def cmd_buy(message: types.Message):
    ...
    try:
        result = await wallet_manager.buy_meme(contract_address, amount_sats, wallet_name)
        # Сохраняем позицию
        add_position(wallet_name, contract_address, amount_sats, "buy")
        await message.answer(
            f"Buy order created!\n"
...
async def cmd_sell(message: types.Message, state: FSMContext = None):
    ...
    try:
        result = await wallet_manager.sell_meme(contract_address, amount_sats, wallet_name)
        # Обновляем позицию
        add_position(wallet_name, contract_address, amount_sats, "sell")
        await message.answer(
...
async def handle_message(message: types.Message, state: FSMContext):
    ...
    # Обработка состояний
        ...
        if current_state == WalletStates.waiting_wallet_name:
            ...
            wallet_manager.set_master_password(password)
            ...
            try:
                ...
                result = wallet_manager.import_wallet(wallet_name, mnemonic)
                await message.answer(
            ...
            try:
                ...
                result = wallet_manager.import_from_private_key(wallet_name, private_key)
            ...
            try:
                ...
                result = wallet_manager.get_private_key(wallet_name, private_key)
                if result:
                    await message.answer(
            ...
            contract = text.strip()
            try:
                if not wallet_manager.validate_spark_btkn_address(contract):
                    await message.answer(
            ...
            try:
                if action == 'buy':
                    result = await wallet_manager.buy_meme(contract, amount, wallet_name)
                else:
                    result = await wallet_manager.sell_meme(contract, amount, wallet_name)
                await message.answer(
...
async def handle_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Обработчик выбора Bitcoin депозита
        ...
        wallet_name, mnemonic, error = get_user_wallet_mnemonic(user_id)
        ...
        saved_address = wallet_manager.get_bitcoin_deposit_address(wallet_name)
        ...
        if saved_address:
            ...
            await callback.message.answer(
                ...
                f"<i>Your Spark address: {wallet_manager.get_active_wallet()[1].address[:20]}...</i>",
                parse_mode="HTML"
            )
            ...
            if result.get('success'):
                ...
                wallet_manager.set_bitcoin_deposit_address(wallet_name, btc_address)
...
ℹ️ Use /help to see all commands"""
    ...
    if data.startswith("set_active_wallet:"):
        ...
        success = wallet_manager.set_active_wallet(wallet_name)
    ...
    # Создание нового кошелька из my_wallets
        ...
        try:
            ...
            wallet_num = len(wallet_manager.wallets)
...
async def handle_buy_callbacks(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Execute - выполнить покупку
        ...
        try:
            ...
            add_position(wallet_name, token_addr, amount_sats, "buy")
    ...
    # Execute Flashnet - выполнить покупку через Flashnet AMM
        ...
        try:
            ...
            # Используем глобальный wallet_manager (определён в начале файла)
    ...
    # Execute Spark - выполнить покупку через Spark Money
        ...
        try:
            ...
            # Используем нативный метод Spark SDK (БЕЗ Flashnet AMM)
            result = await wallet_manager.buy_meme_native(
                contract_address=token_addr,
                amount_sats=amount_sats,
                wallet_name=wallet_name,
                slippage=slippage,
