...
# Глобальный wallet_manager - будет установлен из telegram_bot.py
wallet_manager: SparkWalletManager = None

def set_wallet_manager(manager: SparkWalletManager):
    """Установить wallet_manager из telegram_bot.py"""
    global wallet_manager
    wallet_manager = manager


def get_user_wallet_mnemonic(user_id: int) -> tuple:
    """
    ...
    if not wallet_manager:
        return None, None, "Wallet manager not initialized"
    ...
    wallet_obj = wallet_manager.wallets.get(wallet_name)
